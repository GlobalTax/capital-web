...
interface StepIndicatorMasterProps {
  currentStep: number;
  goToStep: (step: number) => void;
  validateStep: (step: number) => boolean;
}

const StepIndicatorMaster: React.FC<StepIndicatorMasterProps> = ({ currentStep, goToStep, validateStep }) => {
  const { t } = useI18n();
  const steps = [
...
export default StepIndicatorMaster;