            ...
            {steps.map((step, index) => (
              <div key={index} className="relative">
                ...
                <div className="bg-slate-50 rounded-2xl p-8 h-full border border-slate-100 hover:border-slate-200 hover:shadow-lg transition-all duration-300">
                  {/* Number circle */}
                  <div className={`w-16 h-16 rounded-full bg-gradient-to-br from-${accentColor}-500 to-${accentColor}-600 flex items-center justify-center mb-6 shadow-lg relative z-10`}>
                    ...
                    <ul className="space-y-3">
                      {step.features.map((feature, fIndex) => (
                        <li key={fIndex} className="flex items-start gap-3">
                          <div className={`flex-shrink-0 w-5 h-5 rounded-full bg-${accentColor}-100 flex items-center justify-center mt-0.5`}>
                            <Check className={`h-3 w-3 text-${accentColor}-600`} />
                          </div>
