...
export const AddContactsToListDialog: React.FC<AddContactsToListDialogProps> = ({
  open,
  onOpenChange,
  contacts,
  onSuccess,
  ...
  // Mutation to add contacts to list
  const addMutation = useMutation({
    mutationFn: async (listId: string) => {
      const rows = contacts.map(c => ({
        list_id: listId,
        contacto: [c.nombre, c.apellidos].filter(Boolean).join(' '),
        email: c.email || '',
        telefono: c.telefono || '',
        posicion_contacto: c.cargo || '',
        empresa: c.empresa_nombre || '',
        cif: c.empresa_cif || '',
        facturacion: c.empresa_facturacion || null,
        ebitda: c.empresa_ebitda || null,
      }));
      ...
      if (list) {
        await (supabase as any)
          .from('outbound_lists')
          .update({ contact_count: (list.contact_count || 0) + contacts.length })
          .eq('id', listId);
      }
    },
    onSuccess: () => {
      toast({
        ...
        description: `${contacts.length} contacto(s) añadidos a la lista correctamente.`,
      });
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'No se pudieron añadir los contactos.',
        variant: 'destructive',
      });
    },
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {contacts.length} contacto(s) seleccionado(s). Elige la lista destino.
          </DialogDescription>
        </DialogHeader>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {list.lista_madre_id && (
                        <Badge variant="secondary" className="text-xs">Sublista</Badge>
                      )}
                    </div>
                  </div>
                  {selectedListId === list.id && (
          ...
          >
            {addMutation.isPending ? (
              ...
              <>
                ...
                Añadir {contacts.length} contacto(s)
              </>
            )}
          </Button>
        </DialogFooter>
