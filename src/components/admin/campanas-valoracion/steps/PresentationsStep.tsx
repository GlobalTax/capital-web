...
export function PresentationsStep({ campaignId }: PresentationsStepProps) {
  const { presentations, isLoading, uploadFiles, isUploading, assignCompany, autoMatch, isMatching, deletePresentation } = useCampaignPresentations(campaignId);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[], rejectedFiles: any[]) => {
    if (rejectedFiles.length > 0) {
      toast({
        title: 'Solo se aceptan archivos en formato PDF',
        variant: 'destructive',
      });
    }
  ...
  const handleAutoMatch = async () => {
    const candidates = companies.map(c => ({
      id: c.id,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-muted/50 rounded-lg">
        <div className="text-sm">
          <span className="font-semibold">{presentations.length}</span> archivos subidos
        </div>
        ...
        <Button
          ...
          disabled={isMatching || unassigned === 0}
          size="sm"
        >
                ...
                <TableRow key={pres.id}>
                  ...
                  <TableCell>
                    {pres.status === 'assigned' ? (
                      ...
                      <div className="flex items-center gap-2">
                        <Select
                          value={manualAssignments[pres.id] || ''}
                        ...
                        <Button
                          ...
                          onClick={() => handleManualAssign(pres.id)}
                          disabled={!manualAssignments[pres.id]}
                        >
                  ...
                  <TableCell className="text-center">
                    {pres.match_confidence > 0 && (
                      <span className="text-xs text-muted-foreground">
                        {Math.round(pres.match_confidence * 100)}%
                      </span>
                    )}
                  </TableCell>
                  <TableCell className="text-center">
                    {pres.status === 'assigned' && (
                      <Badge variant="default" className="bg-green-600 text-xs">Asignado</Badge>
                    )}
