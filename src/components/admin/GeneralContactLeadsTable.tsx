...
const GeneralContactLeadsTable = () => {
  const { data: leads, isLoading, error } = useQuery({
    queryKey: ['general-contact-leads'],
    queryFn: async () => {
      ...
      return data as GeneralContactLead[];
    }
  });
  ...
  if (error) {
    return (
      <div className="text-center p-8 text-red-600">
        Error al cargar los leads de contacto general
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Leads de Contacto General</h2>
      ...
      {leads?.length === 0 && (
        <div className="text-center p-8 text-gray-500">
          No hay leads de contacto general registrados
        </div>
      )}
    </div>
  );
...
export default GeneralContactLeadsTable;