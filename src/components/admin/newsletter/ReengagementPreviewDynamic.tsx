...
import { ReengagementTemplate } from '@/hooks/useReengagementTemplates';
import { generateReengagementHtml as generateLegacyHtml, ReengagementType } from './templates/reengagementTemplates';
...
interface ReengagementPreviewDynamicProps {
  template: ReengagementTemplate | null;
}

export const ReengagementPreviewDynamic: React.FC<ReengagementPreviewDynamicProps> = ({ 
  template 
}) => {
  ...
  // Get HTML content - use template from DB if available, fallback to legacy
  const htmlContent = useMemo(() => {
    if (!template) return '';
    
    // If template has html_template, use it
    if (template.html_template && template.html_template.trim()) {
      return template.html_template;
    }
    
    // Fallback to legacy generator for system templates without HTML
    const legacySlug = template.slug as ReengagementType;
    return generateLegacyHtml(legacySlug);
  }, [template]);
  ...
  const handleDownloadHtml = () => {
    ...
    a.href = url;
    a.download = `reengagement-${template.slug}-template.html`;
    ...
    toast({
      ...
      description: `reengagement-${template.slug}-template.html`,
    });
  };
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-muted-foreground">
          ...
          <p>Selecciona un tipo de re-engagement para ver la vista previa</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {(template.variables_used || BREVO_VARIABLES.map(v => v.name)).map((variable) => (
