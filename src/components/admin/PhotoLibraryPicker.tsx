...
import { ImageIcon, Search, Folder, ChevronRight, Home } from 'lucide-react';
...
const PhotoLibraryPicker: React.FC<PhotoLibraryPickerProps> = ({ onSelect, trigger }) => {
  ...
  const { photos, folders, isLoading } = usePhotoLibrary(search, currentFolder);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Seleccionar foto de la biblioteca</DialogTitle>
        </DialogHeader>
