...
import { useAdvisorEbitdaMultiplesByRange, AdvisorEbitdaMultiple } from '@/hooks/useAdvisorEbitdaMultiplesByRange';
...
const EbitdaMultiplesManager = () => {
  const { ranges, isLoading, refetch } = useAdvisorEbitdaMultiplesByRange();
  const { toast } = useToast();
  const [showForm, setShowForm] = useState(false);
  const [editingRange, setEditingRange] = useState<AdvisorEbitdaMultiple | null>(null);

  const emptyRange: Omit<AdvisorEbitdaMultiple, 'id'> = {
    sector_name: '',
    range_min: 0,
    range_max: null,
    multiple: 5.0,
  ...
  const groupedRanges = ranges.reduce((acc, range) => {
    ...
    return acc;
  }, {} as Record<string, AdvisorEbitdaMultiple[]>);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-xl font-bold">💰 Múltiplos por EBITDA</h3>
          <p className="text-sm text-muted-foreground">Configura múltiplos según el EBITDA</p>
        </div>
...
export default EbitdaMultiplesManager;
