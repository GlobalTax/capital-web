/**
 * Hook for managing contact lists (listas de contacto)
 * Independent module from outbound campaigns
 */
...
export const useContactLists = () => {
  ...
  const { data: lists = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get last campaign name for each list
      const listIds = (data || []).map((l: any) => l.id);
      let campaignMap: Record<string, string> = {};
      if (listIds.length > 0) {
        const { data: campaigns } = await supabase
          .from('contact_list_campaigns')
          .select('list_id, campaign_nombre')
          .in('list_id', listIds)
          .order('fecha_vinculacion', { ascending: false });
        if (campaigns) {
          for (const c of campaigns as any[]) {
            if (!campaignMap[c.list_id]) {
              campaignMap[c.list_id] = c.campaign_nombre || '—';
            }
          }
        }
      }
  ...
  const duplicateList = useMutation({
    mutationFn: async (id: string) => {
      ...
      const { data: newList, error: createErr } = await supabase
        .from('contact_lists')
        .insert({
          nombre: `${(original as any).nombre} (copia)`,
          descripcion: (original as any).descripcion,
          sector: (original as any).sector,
          origen: (original as any).origen,
          estado: 'borrador',
        })
        .select('id')
        .single();
      ...
      const { data: companies } = await supabase
        .from('contact_list_companies')
        .select('*')
        .eq('list_id', id);
      if (companies && companies.length > 0) {
        const copies = (companies as any[]).map(({ id: _, list_id, created_at, ...rest }) => ({
          ...rest,
          list_id: (newList as any).id,
        }));
        const { error: insertErr } = await supabase.from('contact_list_companies').insert(copies);
...
export const useContactListCompanies = (listId: string | undefined) => {
  ...
  const addCompany = useMutation({
    mutationFn: async (input: Omit<ContactListCompany, 'id' | 'created_at'>) => {
      const { error } = await supabase.from('contact_list_companies').insert(input);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const addCompanies = useMutation({
    mutationFn: async (inputs: Omit<ContactListCompany, 'id' | 'created_at'>[]) => {
      const { error } = await supabase.from('contact_list_companies').insert(inputs);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contact-list-companies', listId] });
      queryClient.invalidateQueries({ queryKey: ['contact-lists'] });
      toast.success('Empresas añadidas');
    },
    onError: (e: Error) => toast.error('Error', { description: e.message }),
  });
  ...
  const deleteCompany = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('contact_list_companies').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contact-list-companies', listId] });
  ...
  const deleteCompanies = useMutation({
    mutationFn: async (ids: string[]) => {
      const { error } = await supabase.from('contact_list_companies').delete().in('id', ids);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contact-list-companies', listId] });
...
export const useContactListCampaigns = (listId: string | undefined) => {
  ...
  const linkCampaign = useMutation({
    mutationFn: async (input: { list_id: string; campaign_id: string; campaign_nombre: string; notas?: string; empresas_enviadas?: number }) => {
      const { error } = await supabase.from('contact_list_campaigns').insert(input);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          created_at,
          list_id,
          contact_lists:list_id (nombre)
        `)
        .eq('empresa', empresaName!)
        .order('created_at', { ascending: false });
      if (error) throw error;
      return (data || []).map((d: any) => ({
        lista: d.contact_lists?.nombre || 'Desconocida',
        fecha: d.created_at,
      }));
    },
  });
