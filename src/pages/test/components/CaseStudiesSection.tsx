    ...
    <section className="py-24 md:py-32 bg-[hsl(0,0%,5%)]">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <span className="text-white/40 text-sm tracking-[0.2em] uppercase block mb-4">
            Casos de éxito
          </span>
        ...
        {isLoading && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-[hsl(0,0%,7%)] p-8">
                <Skeleton className="h-12 w-24 bg-white/10 mb-8" />
                <Skeleton className="h-4 w-32 bg-white/10 mb-4" />
                <Skeleton className="h-8 w-full bg-white/10 mb-4" />
                <Skeleton className="h-16 w-full bg-white/10 mb-6" />
          ...
          >
            {featuredCases.map((caseStudy) => (
              <motion.div
                ...
                className="group bg-[hsl(0,0%,7%)] p-8 hover:bg-[hsl(0,0%,10%)] transition-all duration-300 cursor-pointer"
              >
                {/* Logo */}
                <div className="h-12 mb-8 flex items-center">
                  {caseStudy.logo_url ? (
                    <img 
                      ...
                      alt={caseStudy.title}
                      className="h-full w-auto object-contain filter brightness-0 invert opacity-60 group-hover:opacity-100 transition-opacity"
                    />
                  ) : (
                ...
                {/* Title */}
                <h3 className="font-serif-display text-white text-xl md:text-2xl mb-4 leading-tight group-hover:text-white/90 transition-colors">
                  {caseStudy.title}
                </h3>
                ...
                {/* Hover CTA */}
                <div className="mt-4 flex items-center gap-2 text-white/0 group-hover:text-white/60 transition-all duration-300">
        ...
        <motion.div
          ...
          className="border-t border-white/10 pt-12 mt-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-6"
        >
          <p className="text-white/50 text-lg">
            Más de <span className="text-white font-medium">200 operaciones</span> cerradas con éxito
          </p>
          <Link 
            to="/casos-exito"
            className="inline-flex items-center gap-3 text-white text-sm tracking-wide hover:text-white/70 transition-colors group"
          >
            Ver todos los casos
            <ArrowRight className="w-4 h-4 transform group-hover:translate-x-1 transition-transform" />
          </Link>
