...
BEGIN
  ...
  -- empresa_favorites: uses added_by, not user_id
  DELETE FROM empresa_favorites WHERE empresa_id = p_source_id AND added_by IN (SELECT added_by FROM empresa_favorites WHERE empresa_id = p_target_id);
