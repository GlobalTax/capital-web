-- Insertar templates de ejemplo solo si no existen
INSERT INTO public.landing_page_templates (name, description, type, template_html, template_config, display_order)
SELECT name, description, type::text, template_html, template_config::jsonb, display_order 
FROM (VALUES
  ('Lead Magnet Básico', 'Template para capturar leads con un recurso gratuito', 'generic', 
   ...
   '{"fields": ["title", "description", "cta_text"]}', 1),
  
  ('Calculadora Valoración', 'Landing page para la calculadora de valoración', 'valuation-tool',
   ...
   '{"fields": ["title", "description", "benefit_text", "cta_text"]}', 2),
   
  ('Consulta Sectorial', 'Template para capturar leads por sector específico', 'consultation',
   ...
   '{"fields": ["title", "description", "cta_text"]}', 3)
