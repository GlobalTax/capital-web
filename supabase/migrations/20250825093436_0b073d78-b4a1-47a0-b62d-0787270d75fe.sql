-- SECURITY FIX: Critical Data Exposure Remediation (CORRECTED)
-- Fixed column references for tool_ratings table

-- 1. Clean up ALL existing policies on sensitive tables
-- tool_ratings policies (using correct column names)
DROP POLICY IF EXISTS "Admins can view and manage tool ratings" ON public.tool_ratings;
...
DROP POLICY IF EXISTS "Public tool rating submission with security" ON public.tool_ratings;

-- newsletter_subscribers policies  
DROP POLICY IF EXISTS "Admins full access newsletter subscribers" ON public.newsletter_subscribers;
...
DROP POLICY IF EXISTS "Public newsletter subscription with security" ON public.newsletter_subscribers;
...
-- Secure public insertion for tool ratings with correct column validation
...
WITH CHECK (
  ...
  -- Validate rating fields (correct column names)
  ease_of_use IS NOT NULL AND
  ease_of_use >= 1 AND ease_of_use <= 5 AND
  result_accuracy IS NOT NULL AND
  result_accuracy >= 1 AND result_accuracy <= 5 AND
  recommendation IS NOT NULL AND
  recommendation >= 1 AND recommendation <= 5 AND
  -- Optional email validation (correct column name)
  (user_email IS NULL OR (
    length(TRIM(user_email)) >= 5 AND 
    length(user_email) <= 254 AND
    user_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'
  )) AND
  -- Optional feedback comment validation
  (feedback_comment IS NULL OR length(TRIM(feedback_comment)) <= 1000)
);
...
-- 4. Enhanced security logging (no changes from previous version)
