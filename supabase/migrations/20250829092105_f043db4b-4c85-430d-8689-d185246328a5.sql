...
-- 5. Limpiar cualquier cron job relacionado
SELECT cron.unschedule('abandonment-detector-job') WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'abandonment-detector-job'
);
SELECT cron.unschedule('daily-incomplete-report-job') WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'daily-incomplete-report-job'
);

-- 6. Log de la eliminación
RAISE LOG 'SISTEMA DE VALORACIONES INCOMPLETAS ELIMINADO COMPLETAMENTE: funciones, triggers, tabla, columnas y cron jobs removidos.';